...
1. **必须使用 subagent 无上下文审查**：新建未参与本次实现的子代理，不继承主代理对话历史。启动时显式关闭上下文继承（如工具支持 `fork_turns`，设为 `"none"`）。允许使用通用子代理，不要求专用审查类型。
2. **没有 subagent 就不得自行 commit**：无法启动子代理或无法关闭上下文继承时，报告阻塞；本地自审不能替代独立审查。
3. **必须使用固定提示词**：每次调用前读取 [审查提示词](references/review-prompt.md)，填入调用信息，将正文完整交给子代理。只提供需求、范围、约束与验证记录，不注入主代理的自审结论或预期答案。

等待审查结果后才能提交：仅 `PASS` 放行；`FAIL` 先修复，`BLOCKED` 先补齐核查条件，再新建无上下文子代理复审。审查后若待提交内容发生变化，须重新审查最终内容。提交报告包含审查结论及阻塞项处置结果。
