...
- [ ] **knowledge 承载分类与存量目录迁移** — 散文分类整理纳入 docs/plan-corpus-candidate-batches.md，目标为 base 的设施、通则、资源，以及 guides 的组合、用人、操作、布局。现有 24 篇按分类表移动 18 篇，保留总览与类别/歧义入口；同步 manifest、关联、定位及实际路径消费者，第五批正文使用整理后的路径。当前仍只有设施、通则两处试点目录，完整分类尚未实施；目录整理保留正文语义与检索策略，正文内路径引用一并同步，不据此宣称成本或质量收益。facts/raw 迁移继续由 docs/plan-facts-storage-and-raw-cleanup.md 独立处理。 — 2026-09-19 — 分类方案已补齐，待实施
