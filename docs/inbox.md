...
- [ ] **RAG 父级引导重复展示，送达台账按父级去重低估该重复** — rag_search 返回正文的「父级引导」按命中小节逐行输出，同一父级下的多个子小节同时命中时同一行重复 N 次（实测 2026-09-16 单轮 20 题：正文 79 行对去重 51 种，重复 28 行、2,658 字符，F10 为 5 行同一父级引导）；而 fragmentRanges 按（父级 sectionId, docOffset）去重只登记 1 条，故「命中片段」与内容级复读统计低估该重复，重复占用的 maxContextChars 预算也未计入。相邻「上级范围入口」已有 parentId 去重，两者口径不一致。债务内容与重启条件以代码 TODO(tech-debt) PD-1 为准（bench/src/tool-executor.ts 的 buildSectionContext）。 — 2026-09-16 — 待评估；本轮不改实现，未写 exp、未改基线
