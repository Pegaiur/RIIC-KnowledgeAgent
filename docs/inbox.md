...
- [ ] **独立审定 spec v4 核查草案** — [draft-answer-baseline-v4.md](reports/draft-answer-baseline-v4.md) 已完成全量复核，仍待独立结果审查；审定前不指定正式质量基线。 — 2026-09-09
- [ ] **评估语料实体标记的小规模验证** — 针对名称改写与干员/阵营混用，设计固定检索片段的原文、边界标记、类型标记三组对照，见 [实体标记验证草案（已归档）](archive/draft-entity-marking-probe.md)；原设计基于 lookup/query_operators 路由，当前已统一为 facts_search，重启前需按 schema v5 重估。仅评估，不修改正式语料或启动付费调用。 — 2026-09-07
