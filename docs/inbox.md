...
- [ ] **R5 事实查询（facts-first：LLM 转录记录卡 + lookup/query 查询 tool）** — 在 bench 用真源数据（`knowledge/references/` 唯一真源）落地事实查询工具，量化对 F/G 类题命中率/编造/成本的净效应，产出取代 R4 检索词引导的决策 — 2026-09-02 — 路由 `docs/plan-hybrid-facts.md`（已定稿 plan：LLM 转录 + 程序化核对 + 人工抽检 + 运行时记录卡 + lookup/query 两 tool，经去过度防御收敛；Phase 0【P0.1–P0.7：散文废弃、数据层固化、旧基准标注、meta 删除、工具停用】已完成；**P0.4【F/G 参考要点重制】为实施步骤 3**；Phase 2/3【转录脚本 + agent.ts facts 工具 + 评测】待启动）
