...
- [ ] **名词表（别名/搭配/同名全部返回）** — 阶段 1 契约见 [ADR-010](adr/ADR-010-facts-alias-disambiguation.md)，实施步骤见 [facts 别名与集合词条真源计划](plan-facts-term-alias.md)；2026-09-10 按用户裁决暂不支持简写合称，多人入口仅支持确定阵营与搭配规范名；反问与消歧措施不在范围内，仅保留同名命中全部返回。已修订设计并完成一轮自审，实施待启动；阶段 2（新类别、关系边、数值化）与合称支持后置。两份新 raw 仍不纳入，不修改运行时、不启动付费运行。 — 2026-09-09
