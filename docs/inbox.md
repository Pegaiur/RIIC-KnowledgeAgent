...
- [ ] **优化清洗预检与新导入正文的表格表达** — 源表格便于维护，但长说明行、对齐空白与逐候选重复上下文增加阅读负担。按用户选择纳入 docs/plan-corpus-cleaning-and-pilot.md：预检按源行合并候选并共享完整上下文，规则表行转为带列名的字段表达；新导入正文按内容选择紧凑数值表或自包含条目，保留条件、单位、例外和来源。只在本批清洗与导入层实施，不改查询 Agent 的 RAG/read 送达契约；字符减少不直接等同 token 或问答质量收益。 — 2026-09-18 — 预检优化已实施并验证；正文规则已纳入计划，随两篇正文导入执行
