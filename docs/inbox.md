...
- [ ] **完成 v4 重评草案的全量语义复核** — 独立提交审查连续发现原答误述、额外断言漏检及遗漏/冲突边界疑点；[worker 草案](reports/draft-answer-baseline-v4.md) 已撤回汇总。按固定 spec v4 重新核对 40 份原答、144 项、全部额外断言及逐题送达归因，逐项记录原答依据后才恢复汇总；不改 spec、运行时或知识事实追分，不重新请求模型。 — 2026-09-08
...
- [ ] **确认 harness 性能基线并评估优化方向** — 已完成 2026-09-07 版本 BM25 与 hybrid 各 20 题对照；hybrid facts 实际执行 17 次，同时出现 12 次参数错误。[评估草案](draft-harness-performance.md) 已记录费用、耗时和协议观测，暂留 inbox 待确定优化范围，尚未授权实施优化。 — 2026-09-07
