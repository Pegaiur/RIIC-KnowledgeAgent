...
- [ ] **建立测试架构，约束增量开发漏写/随意测试** — 现状：Vitest 已接入且 `pnpm run test` 已是 merge/release 门禁步骤，bench/tests 有 40 个测试文件、486 个用例（连同 scripts 下测试，全套件 41 文件 493 用例），但 docs/rules/ 无任何测试规则，AGENTS.md 核心约束未涉测试，也没有 src↔test 映射检查；命名、落位、断言风格全靠模仿现有文件，Agent 增量开发时无成文依据。拟先补规则文本：约定覆盖对象与豁免、落位命名、分层断言、增量同步（改行为/修 Bug/新增模块）与禁止事项（不追分、不为转绿放宽断言）。已确认覆盖边界为 bench/src 工具链与 scripts 开发脚本，本轮以规则文本为先、不改门禁。 — 2026-09-12 — 工程草案讨论中，见 docs/draft-testing-architecture.md。定稿后转 plan，并按 document-lifecycle「调整测试策略属横切机制」判据评估 ADR。存量缺口 rate-limiter.ts、cli.ts 是否本轮补测待定。
