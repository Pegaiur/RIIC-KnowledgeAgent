...
- [x] **开发临时产物及时收尾与搜索降噪** — 按如无必要勿增实体收敛：work 共置临时脚本与数据，保留 runs，退役无消费者的 cache/scratch 入口；已完成实施验收，并补齐任务主动收尾及分支遗漏核对规范；旧存量只评估去留，实际清理尚未执行 — 2026-09-06 — → [实施计划](plan-dev-temp-simplification.md)、[实施笔记](plan-dev-temp-simplification-notes.md)

- [x] **共享基准快照更新与淘汰生命周期** — 按严进宽出补齐入库筛选、基线替换、当前目录淘汰及历史证据定位；实施与修复已验收通过，生产原件未清理 — 2026-09-06 — → [实施计划](plan-benchmark-snapshot-lifecycle.md)、[实施笔记](plan-benchmark-snapshot-lifecycle-notes.md)
...
- [x] **基准结果入库与分支收尾清理** — 2026-09-06 按独立评审收敛：每次基准一个 JSON 快照，复用 bench CLI 解析与导出；合并共享后按一份显式 JSON 清单清理，旧存量复用同一格式。不引入多文件快照、自动归属协议或清理发布状态机。实施与修复已验收通过；实际清理尚未执行，正式证据原件仍须满足共享条件 — 2026-09-05 — → [实施计划](plan-temp-cleanup.md)

- [x] **Chat auto 工具循环与每题积分预算** — 已决策使用 Chat、固定 auto、统一工具 schema、可并行的多批调用和每题默认 5 点；最终在 content 作答，程序化 user 消息仅用于未调用工具直接作答的一次回馈兜底。功能实施与修复已验收，质量测评不作为本次门槛，待发布收束 — 2026-09-05 — → [实施计划](plan-agent-auto-tool-budget.md)、[ADR-005](adr/ADR-005-chat-auto-tool-budget.md)
