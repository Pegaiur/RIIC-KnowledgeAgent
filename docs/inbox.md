...
- [x] **knowledge 承载分类与存量目录迁移** — 散文分类整理纳入 docs/plan-corpus-candidate-batches.md，目标为 base 的设施、通则、资源，以及 guides 的组合、用人、操作、布局。现有 24 篇按分类表移动 18 篇，保留总览与类别/歧义入口；同步 manifest、关联、定位及实际路径消费者，第五批正文使用整理后的路径。目录整理保留正文语义与检索策略，正文内路径引用一并同步，不据此宣称成本或质量收益。facts/raw 迁移继续由 docs/plan-facts-storage-and-raw-cleanup.md 独立处理。 — 2026-09-19 — 已验收：18 篇平移、6 篇保持位置，路径消费者（manifest、prose-links、gold、spec、curation、测试、关键词目录）同步，计数仍为 24 篇 / 191 检索块 / 35 定位文档 / 812 定位块；记录见 docs/plan-corpus-candidate-batches-notes.md「步骤 0（散文分类整理）实施与验证」
...
- [ ] **正式 facts 输入须由本仓库脚本可重建** — 2026-09-19 用户明确：knowledge/facts 的 11 份输入不应只以「版本控制内的文本为输入」长期存在；既然留在仓库，就应由本仓库脚本从固定上游解包数据生成，或至少提供可重跑的确定性重建路径。现状与之冲突：生成链在本仓库之外（toolbox-data → RIIC-Web → base-vault），本库从未包含该生成器，ADR-021 决策 2 已定「生成器缺位的事实如实说明，不承诺运行不存在的脚本即可重建」，文件头注即该口径的落地；本仓库唯一参与的脚本只投影九个技能分片的公共练度说明。可复用性与差异评估已完成，见 docs/plan-knowledge-correctness-notes.md「facts 派生链路与 11 份输入对照」（固定提交逐对象对照、字段与 ID 无差异、名册缺四名、等价组 82→84，并记录持有者索引、同名技能 ID、术语尾注等适配项）。待明确：重建粒度（全量生成或生成加投影）、生成器落位与调用命令、是否设「文本可由脚本重现」的门禁、输入格式适配与消费者契约修改的取舍、外部异格名录的唯一源映射，以及是否需修订 ADR-021 决策 2 或另立 ADR。采纳前先裁决与 ADR-021 决策 2 及 docs/plan-knowledge-correctness.md 非目标「不从零重建 facts 派生管道」的冲突。 — 2026-09-19 — 待评估，与上一条「facts 派生接入与正式刷新」合并评估

- [x] **后续散文候选批次** — docs/plan-corpus-candidate-batches.md 承接原知识正确性计划的五批候选，共 35 篇来源（原 36 个对象按实际文件核准），另有 20 篇按需取增量、不整篇导入。五批与散文分类整理均已验收，当前 26 篇正式正文 / 212 检索块，来源、采用、冲突处置与未决项见对应 notes。第五批覆盖练卡取向、布局总表、布局选择、无人机使用、钱书价值与价值比例，以及前批转入的基建投资、长期布局和钱书产出比较；来源固定提交 53cf2f7，上游 main 的布局新增未采用并记入债务。采用与清理沿用写作/术语 spec 和 corpus-addition skill。 — 2026-09-19 — 第五批最终独立审查 PASS，本轮来源与备份已清理；五批以外的候选对象继续留在 inbox，另定范围再实施
