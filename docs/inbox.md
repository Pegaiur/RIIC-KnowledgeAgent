...
- [x] **统一本地依赖工具调用入口** — 在 AGENTS.md 明确使用 `pnpm run`，补充定向测试示例并校正干跑命令；现有门禁与 package scripts 已符合约定，历史故障记录保留。按文档规范调整处理，不单独立 plan/ADR。 — 2026-09-07

- [x] **修复 tooling 测试加载失败** — 移除 `scripts/tooling.mjs` 的 shebang，保持显式 Node 调用，避免 Vite SSR 注入代码后出现语法错误；按小型 Bug 修复处理，不单独立 plan/ADR。 — 2026-09-07
