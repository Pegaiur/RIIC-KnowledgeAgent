...
### 判定 / 待决
- B 组质量显著优（修复模型级失败 F02/F08），但成本 +63%、**平均轮次未降**。
- 与草案通过标准「正确数不降 且 编造数下降 且 平均轮次 ≤ 基线」：质量侧满足、平均轮次侧不满足；成本趋势与「降本」目标相反。
- **暂缓采用**，待决策：①精简 QUERY_GUIDES 降 token 复测（成本主因：规则段 ~700token×44 调用 + 模型多思考）；②接受「质量换成本」；③换 hy3-off 参照（思考更少）。

### 债务记录
- run 产物（gitignore）：A=`2026-09-01T14-59-15-164Z-qwen-low`、B=`2026-09-01T15-04-04-278Z-qwen-low`。
- 本次 A/B 未做全量 20 题逐题人工核查，质量结论为抽查（F02/F08/G01）；严肃结论需补全量核查。
