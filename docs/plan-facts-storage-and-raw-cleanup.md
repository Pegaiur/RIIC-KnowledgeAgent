...
> 实施边界：2026-09-18 用户选择先完善规范与迁移计划；本文已定稿，迁移及删除尚未执行。
...
- 不改变 facts 查询行为、RAG 检索范围、评分口径或历史结果；不增加散文正确性的机械断言。
...
迁移映射为 knowledge/raw/<文件名> → knowledge/facts/<同名文件>。明确清单：名册.md、技能等价组.md、技能-办公室.md、技能-发电站.md、技能-会客室.md、技能-加工站.md、技能-控制中枢.md、技能-贸易站.md、技能-宿舍.md、技能-训练室.md、技能-制造站.md。仅这 11 份转为正式位置，来源稿不随迁移进入 facts。

| 来源材料 | 已知用途与实施处置 |
|---|---|
| raw/上游稿-加工站机制.md、上游稿-训练室机制.md、上游稿-减半.md、上游稿-罗德岛基建建设.md | 已用于设施试点；保留来源仓库、提交及原篇目，修正三篇正式正文的 raw 回指后删除 |
| raw/upstream/上游稿-怪猎中枢.md | 怪猎内容批次已交付；确认现有采用记录具备来源定位后删除 |
| raw/心情消耗恢复与工休时间.md | 已用于既有心情与工休正文；核对尚未采用部分有无明确续作，已完成用途的部分清理 |
| raw/基建工具人汇总表-202605-图片提取.md、raw/PRTS-后勤技能中间产物关系-20260909.md | 旧候选材料；实施时核对是否已确定续作批次。无则删除，有则只保留该批必需部分并记录续作和清理时点；仅有候选登记或笼统的未来待办不构成保留理由 |
...
- [ ] 先完成正式 facts 位置及 raw 排除边界的预期测试，并观察迁移前失败。
- [ ] 11 份正式输入迁至 knowledge/facts，加载器与维护脚本同步，无旧路径回退或重复副本。
- [ ] 当前 gold/spec 与受影响定位同步；事实内容、RAG 范围及评分含义保持不变，历史记录未改写。
- [ ] 8 份来源材料逐项处置：已结束用途的原稿删除，暂留部分有明确续作与清理时点；正式内容不依赖临时稿。
- [ ] 适用的类型、全套测试、构建、完整性、术语、投影、文档与差异检查完成并记录实际结果。
- [ ] 过渡说明收束，ADR 与入口状态同步，实施笔记记录迁移和清理结果。
