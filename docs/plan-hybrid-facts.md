# R5 事实查询（facts-first：确定性记录卡 + 查询 tool）
...
构建**事实查询工具**（零散文依赖）：以 `knowledge/references/`（14 个 md，唯一真源）为源，经 `docs/plan-facts-record-cards.md` 规定的**确定性源解析 + 程序化核对 + 独立人工优化**生成记录卡（名称 / 可用设施 / 技能效果原文 / 代偿备注）；别名、简称、合称和消歧留待后续方案。bench 查询工具（lookup / query_operators）基于记录卡作答，在真源数据上量化事实类题（F/G）的命中率、编造率、成本，为「散文重写 + 判断层补充」之后的完整方案（v3）奠定数据地基。
...
7. **结论落盘**：量化结论写入实施笔记；解析器/人工优化版本与评测模型分别记录；评测 provider ≠ 项目目标 provider Hy3，结论不外推。
...
- [ ] 记录卡字段 + 字段-来源对照表定稿
- [ ] 确定性 references 解析器 + 程序化核对入库（TS，含 fixture 单测）+ 程序化核对 0 差异
- [ ] P0.4 参考要点重制（F/G 批）经人工终审，spec 版本递进
- [ ] 人工抽检完成（语义字段：原文效果 / 代偿备注 / 相关组合；别名与消歧后置）
