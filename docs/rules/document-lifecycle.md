...
| `docs/draft-*.md` | 工程提案草案（未定稿：可行性/选型评估、方案成形讨论） | 立项前评估或方案成形讨论 | 计划定稿前任意修改 | 定稿后重命名为 `plan-*.md` |
...
| `docs/spec/*.md` | 长期内容与核查规格（写作要求、示例、判定口径等**长期复用资产**，非实施蓝图） | 出现需跨任务或跨运行复用的规格时 | 适用范围或规范含义演进时同步维护（文内版本递进，状态由 spec 承载） | 失效或被替代时移入 `docs/archive/` 或删除；生效版本不随 plan 归档 |
| `docs/exp-*.md` / `docs/exp/` | 试验记录（探针、对照、测量、诊断、历史运行分析、答案核查）；`exp/` 子目录存已结束或已取消的记录 | inbox 路由试验工作时 | 按过程、结果、结论三段记录设计、执行、偏离、更正和独立复核情况 | 已结束或已取消后迁入 `docs/exp/`；暂缓保留活动入口且须绑定 inbox 条目 |
...
文档入口见根 `AGENTS.md` 的「文档导航」。draft 仅承载未定稿工程提案，形成实施方案后转 plan；试验工作使用 exp，不为归档虚构 plan，也不另设 report 类型。归口判据（ADR-009）：产出「观察结果」的已设计或已执行工作（探针、对照、测量、诊断、历史运行分析、答案核查）归 exp；产出「方案、建议或待定的决策输入」的可行性/选型评估与工程提案归 draft，其过程含资料调研与技术比较也不例外。历史试验迁移保留原事实、撤回及审定边界，不把旧结论改写成当前事实。既有 plan 保持原类型和正文，历史链接不回改。
...
4. **债务条目落位**：可定位到代码的债务 / backlog，同步在对应代码处写 `TODO(tech-debt) <编号>：` 注释（AGENTS.md 编码核心约束 #5），笔记只登记编号、结论与未来偿还条件，不复制注释细节。
...
`bench/results/*.json` 是可复核的单次运行事实，不是运行目录或成绩台账。此节仅适用于存在独立非 exp 工程用途的快照；入库前必须在对应 plan 或 notes（结果快照的用途登记在根 AGENTS.md「质量基线」表）写明真实消费者和用途。不得以历史文档引用、试验对照或待优化为由保留快照；exp 只保留文内过程、结果和结论。工程用途确需的失败对照与稳定性样本按整体保留，不能只挑成绩好的运行；正式质量基线的记录与机械校验见根 AGENTS.md「质量基线」段——`node dist/cli.js validate` 校验 `bench/results/` 内每份结果都已在该表登记且至少一条，不以一次运行成绩替代答案核查口径。
