...
| `docs/reports/*.md` | 核查报告（固定输入、规格版本及结果证据）；未审定使用 `draft-` 前缀并在 inbox 跟踪 | 生成核查记录时 | 更正并注明原因，或补充复核；草案不得称最终结果 | 被新报告替代后移入 archive，保留撤回说明 |
| `docs/archive/` | 归档（已冻结计划、已结束评估及被替代草案/报告） | 项目初始化 | 计划经脚本归档；非计划记录直接迁入并更新索引与引用 | 不删除 |
...
文档入口见根 `AGENTS.md` 的「文档导航」。草案形成实施方案时转 plan；仅完成评估、已被其他方案承接且没有实施清单的记录直接归档，不为归档虚构 plan。归档日期与原评估日期不同时应在索引注明；历史正文仅修正迁移引用，不把旧结论改写成当前事实。
...
`bench/results/*.json` 是可复核的单次运行事实，不是运行目录或成绩台账。入库前必须在对应 spec、plan 或 notes 中写明该运行支撑的正式结论、对照或仍在进行的实验；dry、探索性探针和重复调试默认不入库。支撑结论的失败对照与稳定性样本按结论整体保留，不能只挑成绩好的运行；没有足够依据时明确写「当前未指定正式质量基线」，不以一次运行成绩替代答案核查口径。

已入库快照必须有当前用途。用途限当前基线、直接比较所需对照和未结束任务的必要证据；用途结束后随正常收尾提交移除，不设额外审批、等待周期、最近 N 份或替换状态协议。每次新增或淘汰时同步核对活动文档引用；历史结论仍需查阅时，使用实际包含该 JSON 的完整提交号和仓库相对路径，并用 `git show <commit>:<path>` 验证可恢复。

快照引用的原件清理继续使用既有显式 JSON 清单：只有清单明确依赖的原件才受快照存在、已提交且干净等保护约束。快照淘汰不要求多人证明本机已清理，也不把未知本机存量变成永久保留理由。清理器、发布技能和本规则不维护额外注册表、生命周期状态或自动删除机制。
