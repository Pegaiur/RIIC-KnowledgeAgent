...
以下通用交接保留约定适用于工程工作；exp 试验只保留文内过程、结果和结论，每次任务结束、暂缓或交接前清理试验附件与一次性脚本，不因后续试验需求继续保存，不先导出快照。历史审定输入仅按 [ADR-009](../docs/adr/ADR-009-exp-document-workflow.md) 的一次性过渡处理。临时原件使用显式清单，已跟踪快照按普通版本控制变更移除；不绕过已有保护，不以更换清单理由规避依赖。具体保护阻塞记入 exp 结论，不另存清理报告。
