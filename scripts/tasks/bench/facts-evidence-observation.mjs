...
选项：
  ...
  --roster <path>   名册真源路径（默认 <仓库根>/knowledge/facts/名册.md；显式指定即视为确认与该运行对应）
...
退出码：0 成功 / 1 一般错误 / 2 参数错误`)
  ...
  const rosterPath = values.roster ? resolve(values.roster) : join(repoRoot, 'knowledge', 'facts', '名册.md')
