 ...
 *
 * RIIC-KnowledgeAgent 适配：单仓（pnpm 根包），门禁 = facts 技能分片投影 + typecheck + test + build + 基准完整性 + doc-check。
...
export const BASE_STEPS = [
  ...
  { id: 'reference-projection', label: 'facts 技能分片公共练度投影', command: ['pnpm', 'run', 'check:reference-projection'] },
