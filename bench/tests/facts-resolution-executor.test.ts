...
function expectEnvelope(item: ToolExecutionResult, expectedIds: string[]) {
  ...
  expect(item.factsResult).toMatchObject({
    factsResultVersion: 7, matchedCount: expectedIds.length, returnedCount: expectedIds.length, complete: true,
  })
...
describe('真实词条的 executor 解析协议', () => {
  ...
  it('临光按登记返回短名自身与长名，substring 路径进入 resolution 与正文', async () => {
    const item = (await executor().executeStep([call('临光')])).results[0]!
    expect(item).toMatchObject({ status: 'success', executed: true })
    expect(item.factsResult?.resolution.items[0]?.paths.map((path) => path.kind)).toEqual(['exact', 'substring'])
    ...
    expect(item.factsResult).toMatchObject({ factsResultVersion: 7, matchedCount: 2, returnedCount: 2, complete: true })
  ...
  it.each(['能天使', '嘉维尔'])('%s 的长名已被阵营精确路径覆盖，不产出 substring 路径', async (query) => {
    const item = (await executor().executeStep([call(query)])).results[0]!
    const paths = item.factsResult!.resolution.items[0]!.paths
  ...
  it('企鹅物流仅返回已登记搭配，不伪造真源中不存在的阵营路径', async () => {
    const item = (await executor().executeStep([call('企鹅物流')])).results[0]!
    expect(item.factsResult?.resolution.items[0]?.paths).toMatchObject([{ kind: 'combo', combo: { id: 'combo:企鹅物流' } }])
    expectEnvelope(item, ['能天使', '德克萨斯', '拉普兰德'])
  })

  it('莱茵科技的开放范围不影响已匹配卡的 complete', async () => {
    const item = (await executor().executeStep([call('莱茵科技')])).results[0]!
    expect(item.factsResult?.resolution.items[0]?.paths).toMatchObject([{
  ...
  it.each(['德狼', '能蕾', '银崖', '孑拉德'])('%s 不作为简写合称接入', async (query) => {
    const item = (await executor().executeStep([call(query)])).results[0]!
    expect(item).toMatchObject({ status: 'empty', executed: true, factsResult: { resolution: { items: [{ index: 0, status: 'empty', paths: [], canonicals: [], message: null }] } } })
    expectEnvelope(item, [])
  })
})
...
describe('错误路径的 executor 边界', () => {
  it('store 加载失败、参数无效及预算拒绝不伪造 resolution', async () => {
    ...
    const tool = createKnowledgeToolExecutor({
      config, query: { id: 'RESOLUTION', category: 'fact', question: '词条协议回归' },
      chunks: [], index: buildIndex([]),
    }, 2)
    const step1 = await tool.executeStep([call(' ', 'invalid')])
    const step2 = await tool.executeStep([call('推王', 'error')])
    const step3 = await tool.executeStep([call('维娜', 'denied')])
    const results = [...step1.results, ...step2.results, ...step3.results]
    expect(results.map((item) => [item.status, item.executed])).toEqual([
      ['invalid_params', false], ['error', true], ['budget_exhausted', false],
    ])
    expect(results[1]?.data).toContain('测试 store 加载失败')
    ...
    expect(step3.snapshot).toMatchObject({ successUsed: 0, attemptUsed: 2, executed: 1, denied: 1, remaining: 2 })
  })
})
