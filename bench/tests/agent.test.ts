...
describe('agent：统一 knowledge 工具 schema', () => {
  ...
  it('人工契约明确记录验收中的设施与组合术语边界', () => {
    ...
    expect(prompt).toContain('回答 F07 时最终答案必须逐项原样出现 `24/x`、`24/y`、`y/x` 三个基线关系')
    expect(prompt).toContain('回答 S02 时必须按火龙 S 黑角、麒麟 R 夜刀、泰拉大陆调查团三名成员逐条覆盖')
    expect(prompt).toContain('木天蓼只按 references 中“可爱的艾露猫/可靠的随从们”两项技能的共享变量解释')
    expect(prompt).toContain('最终答案不得把“木天蓼”与“宿舍恢复”写在同一条因果关系中')
    ...
    expect(prompt).toContain('砾只是 guide 标注的可选非硬核成员，不是红松骑士团成员')
