 ...
 * raw 与 facts 目录既无技能表过滤、也不进入检索与 read。
 */
...
describe('检索范围：manifest 声明的 base/guides', () => {
  it('真实语料检索块严格来自 manifest，且不含 raw 与 facts 条目', () => {
    ...
    expect([...files].filter((file) => file.startsWith('raw/') || file.startsWith('facts/'))).toEqual([])
    expect([...files].every((file) => file.startsWith('base/') || file.startsWith('guides/'))).toBe(true)
  })

  it('模型原文阅读目录同样不含 raw 与 facts，二者都不可由 read 定位', () => {
    ...
    expect(directory.documentRange('raw/心情消耗恢复与工休时间.md')).toBeUndefined()
    expect(directory.documentRange('facts/名册.md')).toBeUndefined()
    expect(directory.documentRange('facts/技能-制造站.md')).toBeUndefined()
