...
import { EQUIVALENCE_FACTS_FILE, FACTS_SOURCE_DIRECTORY, REFERENCE_ROOMS } from './references.js'
...
/** 从仓库真源读取技能等价组（knowledge/facts 下的正式机械真源）。 */
export function loadEquivalenceGroups(root: string, facts: ReferenceFacts): SkillEquivalenceGroup[] {
  try {
    const text = readFileSync(join(root, 'knowledge', FACTS_SOURCE_DIRECTORY, EQUIVALENCE_FACTS_FILE), 'utf-8')
    return parseEquivalenceGroups(text, facts)
  } catch (error) {
    if (error instanceof FactsParseError) throw error
    throw new FactsParseError(`无法读取真源技能等价组：knowledge/${FACTS_SOURCE_DIRECTORY}/${EQUIVALENCE_FACTS_FILE}`)
  }
}
