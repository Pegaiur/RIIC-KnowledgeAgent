...
import { aggregate, renderCsv, renderMarkdown } from './report.js'
import type { BenchQuery, ThinkingMode } from './types.js'
...
function printUsage(): void {
  process.stdout.write(
    [
      'rag-test bench —— 基于 Hy3 的 LLM 查询输出成本基准',
      '',
      '用法：',
      '  node dist/cli.js run [--thinking off|low|high] [--limit N] [--dry] [--questions <path>] [--out <dir>]',
      '  node dist/cli.js report <runDir> [--out <path>]',
      ...
      '  node dist/cli.js run --thinking low           # 真实跑（需 TOKENHUB_API_KEY）',
...
async function main(): Promise<void> {
  ...
  if (args.command === 'run') {
    ...
    process.stdout.write(
      `语料：${stats.files} 个文件｜问题：${picked.length}/${questions.length}｜档位：${args.thinking}｜dry：${args.dry}\n`,
    )
  ...
  if (args.command === 'report') {
    if (!args.runDir) throw new Error('report 需要 runDir 参数（bench/runs/<ts>-<thinking>）')
    const records = readFileSync(join(args.runDir, 'records.jsonl'), 'utf-8')
      .trim()
      .split('\n')
      .filter(Boolean)
      .map((l) => JSON.parse(l))
