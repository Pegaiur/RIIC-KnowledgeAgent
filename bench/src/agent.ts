...
import { callHy3, type ChatMessage, type ProviderOptions } from './provider.js'
...
export function buildSystemPrompt(): string {
  return [
    ...
    '你可以调用 rag_search 检索知识库片段，回答必须基于检索到的内容，无法确认时明确说明。',
    '回答使用中文，结构化排版（要点列表/表格），不要编造数值。',
  ].join('\n')
}
...
export async function runQuery(
  ...
  for (let round = 1; round <= config.maxRounds; round++) {
    rounds++
    const resp = await callHy3(messages, [ragSearchTool()], providerOpts)
    const costs = computeCosts(resp.usage.input, resp.usage.output, resp.usage.cached)
    records.push({
      ts: now,
      queryId: query.id,
